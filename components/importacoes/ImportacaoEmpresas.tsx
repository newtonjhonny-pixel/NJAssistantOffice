...
import {
  Building2, Upload, CheckCircle, XCircle,
 ...
 *
 * Campos reconhecidos: código, razão social, nome fantasia, CNPJ,
 * segmento, matriz/filial, endereço, município, UF, situação.
 *
 * Não duplica CNPJ existente.
 */
...
export default function ImportacaoEmpresas() {
  const [step, setStep] = useState<'upload' | 'preview' | 'done'>('upload')
  const [files, setFiles] = useState<File[]>([])
  const [dragging, setDragging] = useState(false)
  const [loading, setLoading] = useState(false)
  const [preview, setPreview] = useState<{ empresas: EmpresaPreview[]; criar: number; atualizar: number; duplicado: number; erros: number } | null>(null)
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gestao-equipe/importacoes/empresas', { method: 'POST', body: fd })
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      const r = await fetch('/api/gestao-equipe/importacoes/empresas/confirm', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ empresas: preview.empresas.filter(e => e._action !== 'erro' && e._action !== 'duplicado') }),
      })
      const data = await r.json()
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Importação de Empresas</h3>
          <p className="text-sm text-gray-500">Importe empresas e unidades via XLSX, XLS ou CSV. CNPJ não é duplicado.</p>
        </div>
      </div>
      ...
      {step === 'upload' && (
        <div className="space-y-4">
          ...
          <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 text-sm text-blue-700 dark:text-blue-300 flex items-start gap-2">
            ...
            Colunas reconhecidas: <strong>código, razão social, nome fantasia, CNPJ, segmento, matriz/filial, município, UF, situação</strong>.
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 max-h-80 overflow-y-auto">
            <table className="w-full text-xs">
              <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0">
                <tr>
                  {['Ação','CNPJ','Razão Social','Município','UF','Situação'].map(h => (
                    <th key={h} className="px-3 py-2 text-left text-gray-500 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {preview.empresas.map((e, i) => (
                  <tr key={i} className={`bg-white dark:bg-gray-900 ${e._action === 'erro' ? 'opacity-50' : ''}`}>
                    <td className="px-3 py-2">
                      <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${
                        e._action === 'criar' ? 'bg-green-100 text-green-700' :
                        e._action === 'atualizar' ? 'bg-blue-100 text-blue-700' :
                        e._action === 'duplicado' ? 'bg-amber-100 text-amber-700' :
                        'bg-red-100 text-red-700'
                      }`}>{e._action}</span>
                    </td>
                    <td className="px-3 py-2 font-mono text-gray-600">{e.cnpj || '—'}</td>
                    <td className="px-3 py-2 text-gray-800 dark:text-gray-100">{e.razaoSocial}</td>
                    <td className="px-3 py-2 text-gray-500">{e.municipio || '—'}</td>
                    <td className="px-3 py-2 text-gray-500">{e.uf || '—'}</td>
                    <td className="px-3 py-2 text-gray-500">{e.situacao || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleConfirm} disabled={loading || (preview.criar + preview.atualizar) === 0}
              ...
              {loading ? 'Importando...' : `Importar ${preview.criar + preview.atualizar} empresa(s)`}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="max-w-md">
          <div className="p-6 bg-green-50 dark:bg-green-900/20 border border-green-200 rounded-xl text-center space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><p className="text-xs text-gray-500">Criadas</p><p className="font-bold text-green-600">{result.criadas ?? 0}</p></div>
              <div><p className="text-xs text-gray-500">Atualizadas</p><p className="font-bold text-blue-600">{result.atualizadas ?? 0}</p></div>
            </div>
          </div>
