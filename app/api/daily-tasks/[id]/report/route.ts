...
const STATUS_LABELS: Record<string, string> = {
  ...
  NAO_REALIZADO: 'Não realizado', ADIADO: 'Adiado', CANCELADO: 'Cancelado',
}
...
async function buildExcel(id: string): Promise<Buffer> {
  ...
  ws1.addRow(['RELATÓRIO DE ATIVIDADES DIÁRIAS'])
  ws1.getRow(1).font = { bold: true, size: 14 }
  ws1.addRow([])
  ws1.addRow(['Data:', fmtDate(dt.date)])
  ws1.addRow(['Responsável:', dt.responsible ?? ''])
  ws1.addRow(['Título:', dt.title ?? ''])
  ws1.addRow(['Objetivo:', dt.objective ?? ''])
  ws1.addRow(['Status:', dt.status])
  ws1.addRow(['% Concluído:', stats.pct + '%'])
  ws1.addRow([])
  ws1.addRow(['Total planejado:', stats.total])
  ws1.addRow(['Concluídos:', stats.done])
  ...
  ws1.addRow(['Não realizados:', items.filter((i: any) => i.status === 'NAO_REALIZADO').length])
  ws1.addRow(['Cancelados:', stats.cancelled])
  ws1.addRow([])
  ws1.addRow(['Resumo do dia:', dt.summary ?? ''])
  ws1.addRow(['Observações finais:', dt.finalNotes ?? ''])
  ...
  // Aba 3 — Pendências
  const ws3 = wb.addWorksheet('Pendências')
  ws3.columns = [
    { header: 'Atividade',  key: 'title',  width: 30 },
    { header: 'Status',     key: 'status', width: 16 },
    { header: 'Prioridade', key: 'prio',   width: 12 },
...
function buildPDF(data: Awaited<ReturnType<typeof fetchData>>): Buffer {
  ...
  doc.text('Relatório de Atividades Diárias', W / 2, 9, { align: 'center' })
  doc.setFontSize(9); doc.setFont('helvetica', 'normal')
  doc.text(
    `${fmtDate(dt.date)}${dt.responsible ? '  |  ' + dt.responsible : ''}   •   ${stats.pct}% concluído`,
    W / 2, 16, { align: 'center' }
  )
  ...
  doc.text('Título:', 14, y);       doc.setFont('helvetica', 'normal'); doc.text(dt.title ?? '—', 30, y)
  doc.setFont('helvetica', 'bold')
  doc.text('Objetivo:', 14, y + 6); doc.setFont('helvetica', 'normal')
  doc.text(dt.objective ?? '—', 30, y + 6, { maxWidth: W - 44 })
  doc.setFont('helvetica', 'bold')
  doc.text('Status:', 14, y + 14); doc.setFont('helvetica', 'normal'); doc.text(dt.status, 30, y + 14)
  doc.text(`Total: ${stats.total}   Concluídos: ${stats.done}   Pendentes: ${stats.total - stats.done - stats.cancelled}   Cancelados: ${stats.cancelled}`, 70, y + 14)
  y += 30
  ...
  items.forEach((item: any, idx: number) => {
    ...
    if (item.notes) {
      ...
      doc.text(`  ↳ ${item.notes}`, cols[1].x, y, { maxWidth: 150 })
  ...
  doc.text(
    `NJ Assistant Office  •  Gerado em: ${new Date().toLocaleString('pt-BR')}`,
